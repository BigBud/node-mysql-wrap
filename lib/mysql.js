...
MySQL.extend({
	...
	connect: function(DB) {
		...
		MySQL.instance.connectSync(DB.host, DB.user, DB.password, DB.database);
	},
...
MySQL.fn.extend({
	...
	exec: function(query) {
		...
		if (!MySQL.instance.pingSync()) {
			MySQL.connect(MySQL.DB);
		}

		try {
			this.result = MySQL.instance.querySync(query || this.toString());
		} catch (e) {
			console.log(e);
		}
		
		// if query is INSERT return insert id
		if (this.type == 'insert' && this.result) this.result = MySQL.instance.lastInsertIdSync();

		return this.result;
	},

	fetch: function(callback) {
		var result = this.result || this.exec();
